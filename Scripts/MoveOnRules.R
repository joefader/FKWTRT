...
## file names
moveon_tbl <- read.csv("Data/MoveOnRules/MoveOnCompiled/Centroid_95perchull/All_12-240h_25-500k.csv")
moveon_tbl <- read.csv("Data/MoveOnRules/MoveOnCompiled/Centroid_95perchull/S_summer_compiled.csv")
moveon_tbl <- read.csv("Data/MoveOnRules/MoveOnCompiled/Centroid_95perchull/S_winter_compiled.csv")
...
NQ1 %>%
  ...
  ggplot(aes(x = distance, y = days, 
             fill =  100*(1 - (prob_marked_in/max(moveon_tbl$prob_marked_in)))),show.legend = F) + 
  theme_classic() + 
  ...
  geom_text(aes(label = 
          ...
          fontface = "bold"), size = 13, show.legend = F) +  ## add value to tiles
  theme(text = element_text(size=18)) +
  ...
  theme(legend.justification=c(1, 0), legend.position=c(1, .65)) +
  labs(fill='percent 
 reduction') 
